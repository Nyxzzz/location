...
var App = function(){
  ...
  self.db = new mongodb.Db('location', self.dbServer, {auto_reconnect: true});
  ...
  self.routes['root'] = function(req, res){res.send('ABCCDFGG You have come to the park apps web service. All the web services are at /ws/parks*. For example /ws/parks will return all the parks in the system in a JSON payload. Thanks for stopping by and have a nice day'); };

  //returns all the parks in the collection
  self.routes['returnAllParks'] = function(req, res){
    self.db.collection('bigpoints').find().toArray(function(err, names) {
        res.header("Content-Type:","application/json");
        res.end(JSON.stringify(names));
    });
  };

  //returns all the description in the collection
  self.routes['returnAllDescription'] = function(req, res){
    self.db.collection('bigpoints').find({'description'}).toArray(function(err, names) {
        res.header("Content-Type:","application/json");
        res.end(JSON.stringify(names));
    });
  };
  ...
  self.app.post('/ws/description', self.routes['returnAllDescription']);
